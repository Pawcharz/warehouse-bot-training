...
class RolloutBuffer:
  ...
  def add(self, observation, next_observation, action, reward, log_prob, value, done):
...
class PPOAgent:
  ...
  def update(self, obs, actions, old_logprobs, returns, advantages, old_values=None, dones=None, next_obs=None) -> dict:
    ...
    old_params = None
    if self.logger is not None:
      named_model_params = list(self.model.named_parameters())
      old_params = self.logger.capture_parameters(named_model_params)
  ...
  def train(self, env: gym.Env, iterations, early_stopping_fn=None):
    ...
    for i in range(start_iteration, start_iteration + iterations):
      ...
      while True:
        ...
        buffer.add(obs_for_buffer, next_obs_for_buffer, action, reward, logprob, value, done)
        obs = next_obs
      ...
      normalized_rewards = self.reward_normalizer.normalize(np_rewards)
